...
function Select() {
  ...
  const [secondSelectState, setSecondSelectState] = useState(['리액트'])
  ...
  const selectHandler = (item) => {
    const thisSelect = subject.filter((e) => { return item === e })
  ...
  return (
    <>
      <SelectWrap>
        ...
        <div>
          <SelectBtn onClick={firstselectShow}>{firstSelectState} <FontAwesomeIcon icon={faCaretDown} /></SelectBtn>
        </div>
      ...
      <SelectListWrap ref={firstSelect}>
        {
          subject.map((item) => {
            return (
              <FirstSelectList onClick={() => selectHandler(item)}>{item}</FirstSelectList>
            )
          })
        }
      </SelectListWrap>
